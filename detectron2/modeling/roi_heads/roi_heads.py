...
@torch.no_grad()
def get_image_level_gt(weak_classes, num_classes):
    """
    Convert instance-level annotations to image-level
    """
    
    gt_classes_img = weak_classes
...
class Res5ROIHeads(ROIHeads):
    ...
    # this function is based on the function "forward"
    def adapt_ins(self, images, features, proposals, targets):        
        del images
        assert targets
        proposals = self.label_and_sample_proposals(proposals, targets)
        del targets
    ...
    def adapt_img(self, features, proposals, weak_classes):
        ...
        if 0 in num_proposal_per_image:
            return {"loss_img": 0.0 * cls_predictions.sum()}
        ...
        return {"loss_img": img_cls_losses}
